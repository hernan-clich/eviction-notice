...
export function ReplayView({
  ...
  const ordered = useMemo(() => [...transactions].sort((a, b) => a.id - b.id), [transactions]);
  ...
  const schedule = useMemo(
    () => buildReplaySchedule(transactions, snapshots, { bornMs, evictedMs }),
    [transactions, snapshots, bornMs, evictedMs],
  );

  // The whole life's vitals — for the transport curve (full series, peak, seed).
  const fullVitals = useMemo(
    () => computeVitals(transactions, agentState, evictedMs, snapshots),
    [transactions, agentState, evictedMs, snapshots],
  );
  ...
  const aliveState: AgentState = {
    agent_id: agentState?.agent_id ?? 'agent-0',
  ...
  const slicedSnaps = snapshots.filter((s) => Date.parse(s.ts) <= clock.clockMs);
